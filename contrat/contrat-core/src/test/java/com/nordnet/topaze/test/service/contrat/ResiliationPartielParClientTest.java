...
/**
 * C'est la classe test {@link ContratService#resilierProduitParContratParClient(String, String)}.
 * 
 * @author akram-moncer
 * 
 */
public class ResiliationPartielParClientTest extends GlobalTestCase {

	/**
	 * The log of the class.
	 */
	private static final Log LOGGER = LogFactory.getLog(ResiliationPartielParClientTest.class);
	...
	private static PolitiqueResiliation getPolitiqueResiliation() {
		...
		politiqueResiliation.setEnCascade(false);
	...
	public void testResiliationPartielParClientTestValid() {
		...
		try {
			...
			for (ElementContractuel c : contrat.getSousContrats()) {
				if (c.getReferenceProduit().equals("PROD-01")) {
					...
					assertEquals(c.getTypeResiliation(), TypeResiliation.RIC);
				}
			}
		} catch (Exception e) {
			LOGGER.error(e);
	...
	@Test
	@DatabaseSetup(value = { "/dataset/emptyDB.xml", "/dataset/contrat-resiliation-partiel.xml" })
	public void testResiliationPartielParClientContratNotExist() {
	...
	@Test
	@DatabaseSetup(value = { "/dataset/emptyDB.xml", "/dataset/contrat-resiliation-partiel.xml" })
	public void testResiliationPartielParClientContratProduitNotExist() {
		try {
