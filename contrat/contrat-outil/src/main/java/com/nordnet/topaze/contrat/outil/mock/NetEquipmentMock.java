...
public class NetEquipmentMock implements WebServiceNetEquipmentMethods {
	 ...
	 */
	private static Map<String, List<TEquipment>> databaseEquipementClient = new HashMap<>();
	...
	public nordnet.client.ws.netequipment.types.TEquipment[] searchEquipments(
		...
		if (equipmentList == null) {
			...
			for (int i = 0; i <= nbResult; i++) {
				...
				try {
					cl.setTime(PropertiesUtil.getInstance().getDateDuJour().toDate());
				} catch (TopazeException e) {
					LOGGER.error("Error occurs during call of  NetEquipmentMock.searchEquipments()", e);
				}
